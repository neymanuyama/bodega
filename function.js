...
function regionesDelete(codigo) {
    //validamos en este if si queremos eliminar con la confirmación
    if (confirm("¿Esta seguro de querer eliminar?")) {
        var formData = new FormData(document.getElementById("regiones"));
        // .append podemos agregar parametros al formData
        formData.append("metodo", "delete");
        formData.append("codigo", codigo);
        $.ajax({
            url: "functionregiones.php",
            type: "POST",
            dataType: "HTML",
            data: formData,
            asycn: false, //el error que cometí de sintaxis, es async
...
function regionesEditar() {
    regiones.btn.disabled = true;
    regiones.nuevo.disabled = true;
    regiones.modificar.disabled = false;
  
}
function regionesNuevo() {
    regiones.btn.disabled = false;
    regiones.nuevo.disabled = false;
    regiones.modificar.disabled = true;
    //limpia por completo el formulario usando jquery
    $("#regiones")[0].reset();
}

function regionesSelectOne(codigo) {
...
function territoriosDelete(codigo) {
    //validamos en este if si queremos eliminar con la confirmación
    if (confirm("¿Esta seguro de querer eliminar?")) {
        var formData = new FormData(document.getElementById("territorios"));
        // .append podemos agregar parametros al formData
        formData.append("metodo", "delete");
        formData.append("codigo", codigo);
        $.ajax({
            url: "functionterritorios.php",
            type: "POST",
            dataType: "HTML",
            data: formData,
            asycn: false, //el error que cometí de sintaxis, es async
...
function territoriosEditar() {
    territorios.btn.disabled = true;
    territorios.nuevo.disabled = true;
    territorios.modificar.disabled = false;
  
}
function territoriosNuevo() {
    territorios.btn.disabled = false;
    territorios.nuevo.disabled = false;
    territorios.modificar.disabled = true;
    //limpia por completo el formulario usando jquery
    $("#territorios")[0].reset();
}

function territoriosSelectOne(codigo) {
